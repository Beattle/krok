...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $complex_price = $this->request->post['complex-price'];
            if(!empty($complex_price)){
                $this->request->post['price'] = $complex_price;
            }
    ...
    $data['manufacturers'] = $this->model_catalog_manufacturer->getManufacturers();
        ...
        if (isset($this->request->post['product_composition'])) {
            $products = $this->request->post['product_composition'];
        } elseif (isset($this->request->get['product_id'])) {
            $products = $this->model_catalog_product->getProductComposition($this->request->get['product_id']);
        } else {
            $products = array();
        }

		$data['product_compositions'] = array();
        $data['complex_price'] = 0;
        foreach ($products as $product_id) {
            $composition_info = $this->model_catalog_product->getProduct($product_id);

            if ($composition_info) {
                $data['product_compositions'][] = array(
                    'product_id' => $composition_info['product_id'],
                    'name'       => $composition_info['name'],
                    'price'      => $composition_info['price']
                );
            }
            $data['complex_price'] += $composition_info['price'];
        }

      //  die('<pre>'.print_r($products,true).'</pre>');

        $data['complex_products'] = array();
        if(isset($this->request->get['product_id']) && empty($data['product_compositions'])){
            $products = $this->model_catalog_product->getProductComplex($this->request->get['product_id']);
            $data['complex_products'] = array();
            foreach ($products as $product_id){
                $complex_info = $this->model_catalog_product->getProduct($product_id);
                if($complex_info){
                    $data['complex_products'][] = array(
                      'product_id' => $complex_info['product_id'],
                       'name'      => $complex_info['name'],
                       'price'     => $complex_info['price']
                    );
                }
            }
        }

        if(!empty($data['complex_products']))
	...
	public function calcComplex () {

	    $this->load->model('catalog/product');
	    $complex_data = $this->model_catalog_product->calcComplex($this->request->post['product_id'],$this->request->post['price'],$this->request->post['complexes']);
	    $json = $complex_data;
        $this->response->addHeader('Content-Type: application/json');
        $this->response->setOutput(json_encode($json));
    }
