      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-product" class="form-horizontal">
          <ul class="nav nav-tabs">
            ...
            <li><a href="#tab-complex" data-toggle="tab"><?php echo 'Комплект'?></a> </li>
          </ul>
          <div class="tab-content">
            <div class="tab-pane active" id="tab-general">
                <div class="form-group">
                    <label class="col-sm-2 control-label" for="complex-checkbox"><span data-toggle="tooltip" title="<?php echo 'Комплект' ?>"><?php echo 'Комплект'; ?></span></label>
                    <div class="col-sm-10">
                        <input type="checkbox" name="complex_bool" value=""  id="complex-checkbox" class="form-control" />
                    </div>
                </div>
            ...
            <div class="tab-pane" id="tab-data">
              ...
              <div class="form-group">
                  <?php if($complex_products) :?>
                      <div class="col-sm-2">
                          <button data-id="<?php echo $product_id ?>" id="calc-complex" class="btn btn btn-success btn-lg">Сохранить цену и пересчитать комплекты</button>
                      </div>
                  <?php endif;?>
                <label class="col-sm-2 control-label" for="input-price"><?php echo $entry_price; ?></label>
                <div class="col-sm-<?php echo $complex_products?'8':'10';?>">
            ...
            <div class="tab-pane" id="tab-complex">
                <div class="form-group">
                    <label class="col-sm-2 control-label" for="input-complex"><span data-toggle="tooltip" title="<?php echo 'Товары входящие в комплект'; ?>"><?php echo 'Состав'; ?></span></label>
                    <div class="col-sm-10">
                        <input type="text" name="complex" value="" placeholder="<?php echo 'Состав'; ?>" id="input-complex" class="form-control" />
                        <div id="product-complex" class="well well-sm" style="height: 150px; overflow: auto;">
                            <?php foreach ($product_compositions as $composition) { ?>
                                <div data-price="<?php echo $composition['price']?>" id="product-composituib<?php echo $composition['product_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $composition['name']; ?>
                                    <input type="hidden" name="product_composition[]" value="<?php echo $composition['product_id']; ?>" />
                                </div>
                            <?php } ?>
                        </div>
                    </div>
                ...
                <div class="form-group">
                    <label class="col-sm-2 control-label" for="complex-price" >Цена комплекта</label>
                    <div class="col-sm-10">
                        <input class="form-control" id="complex-price" type="text" name="complex-price" value="<?php echo $complex_price;?>"/>
                    </div>
                </div>
                <?php if($complex_products):?>
                <div class="form-group">
                    <div class="row">
                        <h2 class="col-sm-offset-4 col-sm-3">Комплекты</h2>
                    </div>
                    <?php foreach ($complex_products as $complex_product): ?>
                    <div class="row">
                        <div class="input-group col-sm-3 col-sm-offset-4">
                                <input  disabled id="<?php echo $complex_product['product_id'] ?>" class="form-control complexes" value="<?php echo $complex_product['name'] ?>" type="text" aria-describedby="sizing-addon2" title="Комплексный товар">
                                <span class="input-group-addon" id="sizing-addon2"><?php echo $complex_product['price'] ?></span>
                        </div>
                    </div>
                    <?php endforeach;?>
                </div>
...
$('#product-related').delegate('.fa-minus-circle', 'click', function() {
	$(this).parent().remove();
       ...
       // Complex
       var sum_cmplx;
       var $cmplx_prce = $('#complex-price');
       $('input[name=\'complex\']').autocomplete({
           'source': function(request, response) {
               $.ajax({
                   url: 'index.php?route=catalog/product/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
                   dataType: 'json',
           ...
           'select': function(item) {
               ...
               same_prod =  $('#product-composition' + item['value']);
               same_prod.remove();

               $('#product-complex').append('<div data-price="'+item['price']+'" id="product-composition' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="product_composition[]" value="' + item['value'] + '" /></div>');
               if(same_prod.length === 0){

                   sum_cmplx = parseFloat($cmplx_prce.val())+parseFloat(item['price']);
                   $cmplx_prce.val(sum_cmplx);
               }
       ...
       $('#product-complex').delegate('.fa-minus-circle', 'click', function() {

           var $price_rem = $(this).parent().data('price');
           sum_cmplx = parseFloat($cmplx_prce.val())-parseFloat($price_rem);
           $cmplx_prce.val(sum_cmplx);
           $(this).parent().remove();
       });
       
       $('#calc-complex').click(function (e) {
           var price = parseFloat($('#input-price').val());
           e.preventDefault();

           var product_id = $(this).data('id');
           var complexes = $('input.complexes');
           var arrIds = [];
           complexes.map(function () {
              arrIds[arrIds.length] = parseFloat(this.id);
           });

         $.ajax({
              url: 'index.php?route=catalog/product/calcComplex&token=<?php  echo $token; ?>',
              dataType:'json',
              type:'POST',
              data:{price:price,product_id:product_id,complexes:arrIds.join()},
              success:function (json) {
                  Object.keys(json).map(function(key, index) {
                      $('input#'+key).next().text(json[key]);
                  });
              },
              error:function () {
